...
void fill_beters()
{
    ...
    for (int i = 0; i < limit-5; i++)
    {
        if (order_r[i].size() > 0)
        {
            better_r[i].push_back(order_r[i][0].cost);
            for (int j = 1; j < order_r[i].size(); j++)
            {
                better_r[i].push_back(min(better_r[i][j - 1], order_r[i][j].cost));
            }
        }
    }
    // fill better r
    for (int i = 0; i < limit-5; i++)
    {
        int tam = order_l[i].size();
        if (tam > 0)
        {
            for (int j = 0; j < tam; j++)
            {
                better_l[i].push_back(0);
            }
            better_l[i][tam - 1] = order_l[i][tam - 1].cost;
            for (int j = tam - 2; j >= 0; j--)
            {
                better_l[i][j] = min(better_l[i][j + 1], order_l[i][j].cost);
            }
        }
    }
}

int binary_search_l(int pos, int r)
{
    int ini = 0,
        fin = order_l[pos].size() - 1;

    while (ini < fin)
    {
        int mid = ini + (fin - ini) / 2;
        if (order_l[pos][mid].l <= r)
        {
            ini = mid + 1;
        }
        else
...
int main()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (arr[i].num < x && order_r[pos].size() > 0)
        {
            ...
            int pos_l = binary_search_l(pos, arr[i].r),
